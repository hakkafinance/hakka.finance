    ...
    <Box sx={styles.tokenMetricsInfoChain} ml="2">
      ...
      <Box sx={styles.tokenMetricsInfoContainer} mt="20px">
        <Box sx={styles.tokenMetricsInfo}>
          NAME: <span sx={styles.info}>{tokenMetrics.name}</span>
        </Box>
        <Box sx={styles.tokenMetricsInfo} mt="2">
          TYPE: <span sx={styles.info}>{tokenMetrics.type}</span>
        </Box>
        <Box sx={styles.tokenMetricsInfo} mt="2">
          CONTRACT ADDRESS: <br />{' '}
      ...
      <Box sx={styles.tokenMetricsChainList} mt="20px">
        Get HAKKA Token on {tokenMetrics.shortName}:
      </Box>
      { tokenMetrics.id === 'eth'
        ? <Flex sx={styles.token_metric_responsive} mt="2">
          ...
          <Flex>
            ...
            <Box className="right-bot-token">
              <CoinComponent
                whatHakka={true}
                item={{ imageCoin: 'iconBalancer', coinName: 'Balancer', link: 'https://balancer.exchange/#/swap' }}
              />
            </Box>
          </Flex>
        </Flex>
...
function TokenMetrics (props) {
  ...
  const tokenMetrics = [
    ...
    },
    {
      ...
      addressLink: 'https://bscscan.com/address/0x1D1eb8E8293222e1a29d2C0E4cE6C0Acfd89AaaC',
      ...
      return (
        ...
        >
          <Flex className="chain-left-content" alignItems="center">
            <Box sx={{ backgroundColor: chain.imgBg, width: '40px', height: '40px', borderRadius: '8px' }}>
